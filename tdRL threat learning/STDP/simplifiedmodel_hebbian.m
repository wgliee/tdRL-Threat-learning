function V = simplifiedmodel_hebbian
%SIMPLIFIEDMODEL_HEBBIAN Run one microcircuit simulation with Hebbian STDP.
%   V is a 1000-by-100 membrane-potential matrix (time points by neurons).
    ...
    % Five inputs with increasing interneuron drive.
    ...
    for i = 2:totaltime
        for j = 1: neuron_num
            ...
            dS = sum(W_update(1:interneuron_num,j) ...
                .* interactionV(1:interneuron_num)' .* g_inh) ...
                + sum(W_update(interneuron_num+1:neuron_num,j) ...
                .* interactionV(interneuron_num+1:neuron_num)' .* g_ext);
            dV = dV + dS;
            V(i,j) = V(i-1,j) + dV;
            ...
            if V(i-1,j)+dV>V_th && firing(i-1,j)==0
                upper_limit = min([i+tau-1, totaltime]);
            ...
            for m = 1:length(input_cell_idx)
                ...
                if ~isempty(intersect(update_time,firing_time))
                    ...
                    if mean(temp(update_idx))>1
                        W_update(input_cell_idx(m),j) = ...
                            W_update(input_cell_idx(m),j)*max(temp(update_idx));
                    end
                    if mean(temp(update_idx))<1
                        W_update(input_cell_idx(m),j) = ...
                            W_update(input_cell_idx(m),j)*min(temp(update_idx));
                    end
